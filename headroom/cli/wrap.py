...
def _setup_tokensave_mcp_agy(registrar: Any, *, verbose: bool = False) -> bool:
    """Retire stale tokensave entries for agy and report no active tokensave setup.

    Current main retired tokensave in favour of Serena for code-memory MCPs.
    The AGY branch still accepts legacy ``--no-tokensave``/cleanup paths so
    upgraded users stop launching a stale Headroom-installed tokensave server,
    but new AGY wraps should fall through to Serena.
    """
...
def _setup_code_graph(verbose: bool = False) -> bool:
    """Keep the deprecated AGY code-graph setup flag non-fatal.

    The current wrap path forwards ``--code-graph`` to the proxy's live
    code-graph watcher. There is no longer a separate tokensave/CBM MCP to
    register here, so AGY treats this helper as a compatibility no-op.
    """
    if verbose:
        click.echo("  Code graph: using proxy live watcher; no separate MCP setup needed.")
    return True
